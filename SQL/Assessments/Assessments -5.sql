...
SELECT o.OrderID, CONCAT(c.FirstName," ",c.LastName) AS CustomerName, o.ProductID, o.OrderDate, o.Quantity, o.TotalAmount, o.PaymentStatus
...
SELECT o.OrderID, CONCAT(c.FirstName," ",c.LastName) AS CustomerName, p.ProductName, o.OrderDate, o.Quantity, p.Price, o.TotalAmount, o.PaymentStatus
FROM customers c JOIN orders o ON c.CustomerID = o.CustomerID JOIN products p ON p.ProductID = o.ProductID;


-- 4. Retrieve a list of customers who made orders and the order dates.
SELECT CONCAT(c.FirstName," ",c.LastName) AS CustomerName, o.OrderDate FROM customers c JOIN orders o ON c.CustomerID = o.CustomerID;


-- 5. Display the total number of orders placed by each customer.
SELECT CONCAT(c.FirstName," ",c.LastName) AS CustomerName, COUNT(o.OrderID) FROM customers c JOIN orders o ON c.CustomerID = o.CustomerID GROUP BY c.CustomerID; 


-- 6. Retrieve a list of products and their manufacturers.
SELECT ProductName, Manufacturer FROM products;


-- 7. Display the orders along with the product names and quantities ordered.
SELECT o.OrderID, CONCAT(c.FirstName," ",c.LastName) AS CustomerName, p.ProductName, o.OrderDate, o.Quantity, o.TotalAmount, o.PaymentStatus
FROM customers c JOIN orders o ON c.CustomerID = o.CustomerID JOIN products p ON p.ProductID = o.ProductID;


-- 8. List the customers who have not placed any orders.
SELECT CONCAT(c.FirstName," ",c.LastName) AS CustomerName FROM customers c WHERE c.CustomerID NOT IN (SELECT CustomerID FROM orders); 
