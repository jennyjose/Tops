...
ALTER TABLE Flights ADD FlightStatus VARCHAR(15) DEFAULT 'Scheduled';
SELECT * FROM Flights;
...
UPDATE Flights SET TicketPrice = 1500.00 WHERE DepartureCity LIKE "London";
SELECT * FROM Flights;


-- 7) Delete all flights that have an arrival city of 'Paris'.
DELETE FROM Flights WHERE ArrivalCity LIKE "Paris";
SELECT * FROM Flights;


-- 8) Add a new column 'DepartureTerminal' with a default value of 'T1'.
ALTER TABLE Flights ADD DepartureTerminal VARCHAR(15) DEFAULT "T1";
SELECT * FROM Flights;
...
UPDATE Flights SET FlightStatus = "Boarding" WHERE DepartureCity LIKE "New York";
SELECT * FROM Flights;
