...
SELECT category, COUNT(sale_id) FROM sales GROUP BY category HAVING COUNT(sale_id) > 10;
...
SELECT product_name, COUNT(sale_id) FROM sales GROUP BY product_name;
...
SELECT product_name, AVG(sale_amount) FROM sales GROUP BY product_name HAVING COUNT(sale_id) >2;
...
SELECT product_name, SUM(sale_amount) FROM sales GROUP BY product_name ORDER BY SUM(sale_amount) DESC LIMIT 1;
...
SELECT MONTH(sale_date) AS sales_month, AVG(sale_amount) FROM sales GROUP BY sales_month;

-- 12-	What are the products with the highest sale amount in each category?
SELECT category, product_name, MAX(sale_amount) FROM sales GROUP BY category, product_name;

-- 13-	How many sales occurred for each month?
SELECT MONTH(sale_date), COUNT(sale_id) FROM sales GROUP BY MONTH(sale_date);

-- 14-	In which months are there more than 5 sales?
SELECT MONTH(sale_date), COUNT(sale_id) FROM sales GROUP BY MONTH(sale_date) HAVING COUNT(sale_id) > 5;

-- 15-	What is the average sale amount for each product category in months with more than 3 sales?
SELECT MONTH(sale_date), product_name, AVG(sale_amount) FROM sales GROUP BY MONTH(sale_date),product_name HAVING COUNT(sale_id) > 3;
...
SELECT MONTH(sale_date), SUM(sale_amount) FROM sales GROUP BY MONTH(sale_date) ORDER BY SUM(sale_amount) LIMIT 1;

-- 18-	Which products have sale amounts that never go below 50?
SELECT product_name, SUM(sale_amount) FROM sales GROUP BY product_name HAVING SUM(sale_amount) >50;

-- 19-	Which category has the highest total sales amount in each month?
SELECT MONTH(sale_date), category, SUM(sale_amount) FROM sales GROUP BY MONTH(sale_date), category ORDER BY SUM(sale_amount) DESC;

-- 20-	Which products have sale amounts above the average sale amount across all products?
SELECT product_name, SUM(sale_amount), AVG(sale_amount) FROM sales GROUP BY product_name HAVING SUM(sale_amount) > AVG(sale_amount);
...
SELECT MONTH(sale_date), (MAX(SUM(sale_amount)) - MIN(SUM(sale_amount))) / MIN(SUM(sale_amount)) AS sales_increase_percentage
FROM sales
GROUP BY MONTH(sale_date)
HAVING sales_increase_percentage > 0.20;

-- 26-	Which products have more than 2 sales below 30 in any month?
-- SELECT product_name, MONTH(sale_date), SUM(sale_amount) FROM sales GROUP BY product_name, MONTH(sale_date) HAVING COUNT(*) > 2 AND SUM(sale_amount) <30;
SELECT product_name
FROM sales
...
SELECT category, product_name, SUM(sale_amount) FROM sales GROUP BY category, product_name HAVING category = "Electronics"  AND category = "Clothing";
SELECT product_name
FROM (
...
SELECT category, SUM(sale_amount) FROM sales GROUP BY category HAVING (COUNT(sale_amount) >1 AND SUM(sale_amount) > 100) AND (COUNT(sale_amount) >1 AND SUM(sale_amount) > 100);
SELECT category
FROM sales
GROUP BY category
