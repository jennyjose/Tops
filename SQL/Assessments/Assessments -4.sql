...
SELECT * FROM cars WHERE Price > (SELECT AVG(Price) FROM cars) AND FuelType LIKE "Gasoline";
...
SELECT Model FROM cars WHERE EngineType LIKE "V8" GROUP BY Model ORDER BY MIN(Price) LIMIT 1;
...
SELECT c1.* FROM cars c1 JOIN cars c2 ON c1.CarID = c2.CarID WHERE c1.Price > (SELECT AVG(Price)
    FROM Cars) AND c1.TransmissionType = c2.TransmissionType AND c1. FuelType = c2.FuelType; 
